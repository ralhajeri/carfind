...
import { runDatabaseTestSuite, validateDatabaseService, performanceTest } from './lib/services/database-test-utils';
...
async function testDatabaseServiceLayer() {
    try {
        ...
        if (isConfigured) {
            ...
            console.log(`   ${integrationValid ? '✅' : '❌'} Database service integration: ${integrationValid ? 'PASSED' : 'FAILED'}\n`);
            ...
            if (integrationValid) {
                ...
                if (testResults.overall.success) {
                    ...
                    console.log(`   ✅ All tests passed: ${testResults.overall.passedTests}/${testResults.overall.totalTests}`);
                    console.log(`   ✅ Total duration: ${testResults.overall.totalDuration}ms`);
                    console.log(`   ✅ Average session create: ${perfResults.avgSessionCreate.toFixed(2)}ms`);
                    console.log(`   ✅ Average message create: ${perfResults.avgMessageCreate.toFixed(2)}ms`);
            ...
            console.log('⚠️ Database not configured - this is expected if environment variables are not set');
