...
test.describe('CarFind AI Recommendation Testing (REQ-003)', () => {
    ...
    test('TEST-F-003-1: Family car recommendations with fuel economy', async () => {
        ...
        await chatPage.sendUserMessage('I need a family car with good fuel economy');
    ...
    test('TEST-F-003-2: New driver car recommendations', async () => {
        ...
        if (content) {
            ...
            expect(content).toMatch(/(safety|reliable|affordable|budget|beginner|new driver)/i);
    ...
    test('TEST-F-003-4: Follow-up detail request', async () => {
        ...
        if (content) {
            ...
            expect(content).toMatch(/(year|price|mileage|color|features|description)/i);
    ...
    test('TEST-F-003-5: Commuting car recommendations', async () => {
        ...
        if (content) {
            ...
            expect(content).toMatch(/(commut|fuel.*efficien|electric|hybrid|econom)/i);
    ...
    test('TEST-F-003-7: Multiple constraint recommendations', async () => {
        ...
        await chatPage.sendUserMessage('I want an electric car under $40,000 for family use');
        await chatPage.isGenerationComplete();
    ...
    test('TEST-F-003-8: Response quality and format validation', async () => {
        ...
        if (content) {
            ...
            expect(content).toMatch(/(recommend|suggest|help|need|budget|use|prefer)/i);
        }
    });
});