...
export abstract class BaseAIService implements AIService {
    ...
    protected validateConfig(config: AIServiceConfig): void {
        if (!config.apiKey) {
            throw ErrorFactory.service('AIService', 'configuration', 'API key is required for AI service initialization');
        }

        if (!config.model) {
            throw ErrorFactory.service('AIService', 'configuration', 'Model identifier is required for AI service initialization');
        }

        // Validate optional numeric parameters
        if (config.maxTokens !== undefined && (config.maxTokens <= 0 || !Number.isInteger(config.maxTokens))) {
            throw ErrorFactory.validation('maxTokens', config.maxTokens, 'positive integer', 'Max tokens must be a positive integer');
        }

        if (config.temperature !== undefined && (config.temperature < 0 || config.temperature > 2)) {
            throw ErrorFactory.validation('temperature', config.temperature, 'range 0.0-2.0', 'Temperature must be between 0.0 and 2.0');
        }
    }
