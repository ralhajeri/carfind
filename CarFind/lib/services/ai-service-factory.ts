...
export class AIServiceFactory {
    private static serviceRegistry = new Map<AIServiceType, AIServiceConstructor>();
     ...
     */
    static registerService(type: AIServiceType, serviceClass: AIServiceConstructor): void {
     ...
     */
    static getServiceClass(type: AIServiceType): AIServiceConstructor | undefined {
     ...
     */
    static validateConfig(type: AIServiceType, config: AIServiceConfig): {
        isValid: boolean;
        issues: string[];
    } {
        const issues: string[] = [];
     ...
     */
    private static validateOpenAIConfig(config: AIServiceConfig, issues: string[]): void {
        // OpenAI model validation
        const supportedModels = ['gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo', 'gpt-3.5-turbo', 'o1-mini', 'o1-preview'];
        const isModelSupported = supportedModels.some(supported => config.model.includes(supported));

        if (!isModelSupported) {
            issues.push(`Model '${config.model}' may not be supported by OpenAI. Supported models: ${supportedModels.join(', ')}`);
        }
        ...
        if (config.maxTokens !== undefined && (config.maxTokens <= 0 || config.maxTokens > 128000)) {
     ...
     */
    static createWithValidation(type: AIServiceType, config: AIServiceConfig): AIService {
    ...
    static getFactoryInfo(): {
        ...
        const supportedCapabilities: Record<AIServiceType, string[]> = {} as Record<AIServiceType, string[]>;
