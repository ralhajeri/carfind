...
import { DatabaseService, createDatabaseService, getDatabaseService } from './database-service';
 ...
 */
export async function testSessionOperations(service?: DatabaseService): Promise<DatabaseTestResult> {
    const startTime = Date.now();
    ...
    try {
        ...
        if (!Array.isArray(userSessions) || !userSessions.some(s => s.id === testSessionId)) {
 ...
 */
export async function testMessageOperations(sessionId: string, service?: DatabaseService): Promise<DatabaseTestResult> {
    const startTime = Date.now();
    const dbService = service || getDatabaseService();
    ...
    try {
        ...
        if (!Array.isArray(sessionMessages) || !sessionMessages.some(m => m.id === testMessageId)) {
        ...
        return {
            ...
            duration: Date.now() - startTime,
            details: { messageId: testMessageId, messageCount: sessionMessages.length }
        };
    } catch (error) {
        return {
            success: false,
 ...
 * Clean up test data
 */
export async function cleanupTestData(sessionId: string, service?: DatabaseService): Promise<DatabaseTestResult> {
    const startTime = Date.now();
    const dbService = service || getDatabaseService();
 ...
 */
export async function runDatabaseTestSuite(service?: DatabaseService): Promise<DatabaseTestSuite> {
    const dbService = service || getDatabaseService();
    const testSessionId = `test-session-${Date.now()}`;
    ...
    console.log(`   ${connectionTest.success ? '✅' : '❌'} ${connectionTest.message} (${connectionTest.duration}ms)\n`);
    ...
    console.log(`   ${sessionTest.success ? '✅' : '❌'} ${sessionTest.message} (${sessionTest.duration}ms)\n`);
    ...
    if (sessionTest.success && sessionTest.details?.sessionId) {
        ...
        messageTest = await testMessageOperations(sessionTest.details.sessionId, dbService);
        console.log(`   ${messageTest.success ? '✅' : '❌'} ${messageTest.message} (${messageTest.duration}ms)\n`);

        // Test 4: Cleanup
        console.log('4️⃣ Testing cleanup operations...');
        cleanupTest = await cleanupTestData(sessionTest.details.sessionId, dbService);
        console.log(`   ${cleanupTest.success ? '✅' : '❌'} ${cleanupTest.message} (${cleanupTest.duration}ms)\n`);
    } else {
        messageTest = {
            success: false,
    ...
    console.log(`📊 Test Results: ${passedTests}/${tests.length} passed (${totalDuration}ms total)`);
    console.log(`${overallSuccess ? '🎉' : '⚠️'} Database Service Test Suite ${overallSuccess ? 'PASSED' : 'FAILED'}\n`);
...
export async function validateDatabaseService(): Promise<boolean> {
    try {
        ...
        if (!connectionTest.success) {
            console.warn('Database connection validation failed:', connectionTest.message);
            return false;
        }
...
export async function performanceTest(iterations: number = 10): Promise<{
    ...
    console.log(`   Session Create: ${results.avgSessionCreate.toFixed(2)}ms avg`);
    console.log(`   Session Read: ${results.avgSessionRead.toFixed(2)}ms avg`);
    console.log(`   Message Create: ${results.avgMessageCreate.toFixed(2)}ms avg`);
