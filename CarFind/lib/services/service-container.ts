...
export class ServiceContainer {
  ...
  getServiceWithFallback(type: AIServiceType, sessionId?: string): AIService {
    try {
      ...
      if (type !== this.defaultServiceType) {
        console.warn(
          `Service type '${type}' not available, falling back to default '${this.defaultServiceType}'`,
        );
  ...
  validateConfiguration(): {
    ...
    if (!this.isServiceConfigured(this.defaultServiceType)) {
      issues.push(
        `Default service type '${this.defaultServiceType}' is not configured`,
      );
    }
    ...
    this.getAvailableServiceTypes().forEach((type) => {
      if (!AIServiceFactory.isServiceSupported(type)) {
        warnings.push(
          `Service type '${type}' is configured but not supported by factory`,
        );
      }
    });
  ...
  async performHealthCheck(): Promise<
    Record<AIServiceType, { healthy: boolean; error?: string }>
  > {
