...
export class OpenAIService extends BaseAIService {
  ...
  protected validateConfig(config: AIServiceConfig): void {
    ...
    if (
      ...
      console.warn(
        'Warning: Using non-standard OpenAI API endpoint:',
        config.baseUrl,
      );
    }
    ...
    if (
      !supportedModels.some((supported) => config.model.includes(supported))
    ) {
      console.warn(
        'Warning: Model may not be supported by OpenAI:',
        config.model,
      );
    }
  }
