...
export async function createDatabaseService(useServerClient: boolean = false): Promise<DatabaseService> {
    const client = useServerClient ? await createServerSupabaseClient() : createClient();
