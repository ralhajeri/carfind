...
export class StreamingValidationTests {
        ...
        try {
            ...
            for await (const chunk of streamGenerator) {
                ...
                if (typeof chunk !== 'string') {
                    errors.push(`Chunk ${metrics.totalChunks} is not a string: ${typeof chunk}`);
                }

                // Validate chunk is not empty (for meaningful responses)
                if (chunk.trim().length === 0) {
                    console.log(`⚠️  Empty chunk received at position ${metrics.totalChunks}`);
                }
            }
            ...
            console.log(`✅ Streaming completed: ${metrics.totalChunks} chunks in ${metrics.totalResponseTime}ms`);
            ...
            if (metrics.firstChunkTime > 5000) {
                errors.push(`First chunk took too long: ${metrics.firstChunkTime}ms (>5s)`);
            }

        } catch (error) {
            errors.push(`Streaming test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
        ...
        try {
            ...
            const request: ChatRequest = {
                messages: [{
                    id: 'test-tool-1',
                    role: 'user',
                    content: 'Search for Toyota cars under $30,000 and tell me about the results.',
                    timestamp: new Date()
                }],
            ...
            errors.push(`Tool streaming test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
        ...
        try {
            ...
            try {
                ...
                for await (const chunk of streamGenerator) {
                    ...
                    errors.push('Received chunks with invalid API key - error handling failed');
                }
        ...
        try {
            ...
            for (const config of configs) {
                try {
                    ...
                    errors.push(`Service ${servicesCreated + 1} creation/streaming failed: ${serviceError instanceof Error ? serviceError.message : 'Unknown error'}`);
                }
            }

        } catch (error) {
            errors.push(`Factory consistency test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
     ...
     */
    private static generateValidationSummary(testResults: Record<string, any>, allPassed: boolean): string {
        ...
        // Collect all errors
        const allErrors = Object.values(testResults).flatMap(result => result.errors);
