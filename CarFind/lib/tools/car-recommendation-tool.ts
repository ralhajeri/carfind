...
export const carRecommendationTool = tool({
  description:
    'Get personalized car recommendations based on user needs, budget, and preferences. Use this for advisory conversations.',
  inputSchema: z.object({
    budget: z.number().positive().optional().describe('User budget in USD'),
    usage: z
      .enum(['commuting', 'family', 'luxury', 'work', 'sport'])
      .optional()
    ...
    features: z
      ...
      .optional()
      .describe(
        'Important features (e.g., fuel-efficient, spacious, reliable)',
      ),
  }),
  async execute(input) {
    try {
      ...
      if (input.usage) {
        switch (input.usage) {
          ...
          case 'family':
            recommendations = recommendations.filter(
              (car) =>
                ['SUV', 'Minivan'].some((type) =>
                  car.description.includes(type),
