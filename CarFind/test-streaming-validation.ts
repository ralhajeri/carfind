...
class StreamingValidator {
    ...
    private async waitForServerReady(): Promise<void> {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            console.log(`   🔄 Attempt ${attempt}/${maxAttempts}: Server not ready yet, waiting...`);
        ...
        try {
            ...
            if (homeResponse.ok) {
                ...
                if (cookies) {
                    ...
                    console.log('⚠️ No session cookies received, tests may need manual session');
                }
            } else {
                console.log('⚠️ Home page request failed, proceeding without auth');
            }
        } catch (error) {
            console.log('⚠️ Auth initialization failed, will test without authentication:', error);
        }
    }
    ...
    private async testBasicStreamingPerformance(): Promise<void> {
        ...
        try {
            ...
            const timeToFirstByte = firstByteTime ? (firstByteTime - startTime) / 1000 : 0;
            const avgChunkInterval = chunkTimes.length > 1 ?
                (chunkTimes[chunkTimes.length - 1] - chunkTimes[0]) / (chunkTimes.length - 1) : 0;
            ...
            console.log(`   ✅ Result: ${passed ? 'PASSED' : 'FAILED'} - ${timeToFirstByte.toFixed(2)}s to first byte\n`);
    ...
    private async testCarSearchToolStreaming(): Promise<void> {
        ...
        try {
            ...
            const response = await this.makeAuthenticatedRequest(`${this.baseUrl}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    message: {
                        ...
                        role: 'user',
                        parts: [{ type: 'text', text: 'Show me Toyota cars under $30,000' }],
                        createdAt: new Date().toISOString(),
                    },
                    selectedChatModel: 'chat-model',
                    selectedVisibilityType: 'private',
            ...
            try {
                while (true) {
                    ...
                    if (chunk.includes('searchCars') || chunk.includes('tool')) {
                        ...
                        console.log(`   🔧 Tool invocation detected in chunk ${chunkCount}`);
                    }
            ...
            const totalTime = performance.now() - startTime;
            const passed = toolInvocationDetected && streamingContinuedAfterTool && chunkCount > 0;
            ...
            console.log(`   ✅ Result: ${passed ? 'PASSED' : 'FAILED'} - Tool integration streaming\n`);
    ...
    private async testComplexQueryStreaming(): Promise<void> {
        ...
        try {
            const complexQuery = 'Compare Toyota Camry vs Honda Civic for commuting, considering fuel economy, reliability, maintenance costs, and safety ratings. Provide detailed analysis.';
            const startTime = performance.now();
            let firstByteTime: number | undefined;
            let chunkCount = 0;
            let totalBytes = 0;
            ...
            try {
                while (true) {
                    ...
                    const chunk = decoder.decode(value);
                    console.log(`   📦 Complex Chunk ${chunkCount}: ${chunk.substring(0, 50)}...`);
                }
            } finally {
                reader.releaseLock();
            }
            ...
            console.log(`   ✅ Result: ${passed ? 'PASSED' : 'FAILED'} - Complex query streaming\n`);
        ...
        try {
            ...
            try {
                while (true) {
                    ...
                    console.log(`   📦 SSE Chunk ${chunkCount}: ${chunk.substring(0, 100)}...`);
                }
            } finally {
                reader.releaseLock();
            }
            ...
            console.log(`   ✅ Result: ${passed ? 'PASSED' : 'FAILED'} - SSE format validation\n`);
        ...
        try {
            ...
            const response = await this.makeAuthenticatedRequest(`${this.baseUrl}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    message: {
                        ...
                        role: 'user',
                        parts: [{ type: 'text', text: 'Write a long response about electric vehicles' }],
                        createdAt: new Date().toISOString(),
                    },
                    selectedChatModel: 'chat-model',
                    selectedVisibilityType: 'private',
            ...
            try {
                while (true) {
                    ...
                    console.log(`   📦 Interrupt Chunk ${chunkCount}: ${chunk.substring(0, 50)}...`);

                    // Simulate interruption after a few chunks
                    if (chunkCount === 3) {
                        console.log(`   ⚠️ Simulating interruption after chunk ${chunkCount}`);
                        break;
                    }
                }
            ...
            console.log(`   ✅ Result: ${passed ? 'PASSED' : 'FAILED'} - Stream interruption handling\n`);
...
`;
            ...
            if (result.metrics) {
                report += `   Metrics: `;
                if (result.metrics.firstByteTime) report += `First Byte: ${result.metrics.firstByteTime.toFixed(2)}s, `;
                if (result.metrics.totalTime) report += `Total: ${result.metrics.totalTime.toFixed(2)}s, `;
                if (result.metrics.chunkCount) report += `Chunks: ${result.metrics.chunkCount}`;
                report += '\n';
            }
        });
